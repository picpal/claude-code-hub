...
### 5. MCP Tool (MCP 서버 도구 호출)

이미 연결된 MCP 서버의 도구를 호출합니다. 도구의 텍스트 출력은 command 훅의 stdout처럼 처리됩니다.
...
> `agent` 타입은 실험적(experimental) 기능으로 동작과 설정이 변경될 수 있습니다.
...
| `Setup` | `--init-only`/`-p` 초기화 모드로 시작할 때 (1회성 준비) |
...
| `PostToolBatch` | 병렬 도구 호출 묶음이 모두 끝났을 때 |
...
| `UserPromptExpansion` | 입력한 슬래시 명령이 프롬프트로 확장될 때 |
...
| `StopFailure` | API 오류로 턴이 종료되었을 때 |
...
`if` 필드는 **권한 규칙(permission rule) 문법**으로 도구 이벤트에서만 훅 실행을 좁힙니다. 정확히 하나의 규칙만 가지며 `==`, `{{ }}`, `||` 같은 표현식 문법은 지원하지 않습니다. `PreToolUse`, `PostToolUse`, `PostToolUseFailure`, `PermissionRequest`, `PermissionDenied` 이벤트에서만 평가됩니다. (여러 도구를 OR로 묶으려면 그룹 수준의 `matcher`를 사용합니다.)
