...
| 2 | **Local** | `.claude/settings.local.json` | 개인 프로젝트 설정 (gitignore) |
| 3 | **Project** | `.claude/` (Git 커밋) | 프로젝트 공통 설정 |
| 4 (최저) | **User** | `~/.claude/` | 개인 전역 설정 |

> CLI 인자(예: `--permission-mode`)는 Managed 다음, Local보다 위의 우선순위로 적용됩니다. 즉 동일 항목이 여러 범위에 있으면 **Managed > CLI 인자 > Local > Project > User** 순으로 우선합니다.
...
| `voiceEnabled` | 음성 입력 활성화 (레거시 — `voice.enabled` 권장) | `true` / `false` |
...
| `disableAutoMode` | Auto Mode 비활성화 | `"disable"` |
...
| `showTurnDuration` | 턴 소요 시간 표시 | `true` |
