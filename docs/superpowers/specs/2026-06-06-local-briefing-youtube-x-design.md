...
- **TCC: 런타임을 Desktop 밖으로 이전(필수).** macOS TCC가 launchd 백그라운드 프로세스의 `~/Desktop`(보호 폴더) 접근을 차단해 첫 launchd 실행이 `Operation not permitted`로 즉사함. 해결: `scripts/deploy-local.sh`가 런타임(scripts+config+x_endpoints+venv+state+.env)을 **`~/claudehub-briefing`(비보호 경로)** 로 복사. 개발 repo는 Desktop에 그대로, launchd는 배포본을 실행. 코드 변경 시 `deploy-local.sh` 재실행으로 재동기화(드리프트 방지). plist는 배포 경로를 가리킴.
- **남은 사용자 작업.** (1) `bash scripts/deploy-local.sh`(완료됨), (2) `~/claudehub-briefing/.env`에 실제 키 입력, (3) `launchctl unload ...; launchctl load -w ~/Library/LaunchAgents/com.claudehub.briefing.plist`, (4) `launchctl kickstart -k gui/$(id -u)/com.claudehub.briefing` 1회 후 `~/claudehub-briefing/.briefing-state/launchd.out.log`에서 X 라인 유무로 **launchd 컨텍스트 Keychain 접근 검증**(실패 시 쿠키 파일 폴백).
